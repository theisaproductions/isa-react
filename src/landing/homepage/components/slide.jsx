            ...
            <div className='h-screen relative cursor-crosshair'>
                    ...
                    <div className='absolute top-0 left-0 h-full w-full z-10 p-5vw'>
                        <div className='h-full w-full flex flex-col items-center justify-center gap-10 animate-shrinkGap'>
                            <img className="w-1/2 md:w-1/3 lg:w-1/4 animate-fadeIn drop-shadow-lg" src={`${process.env.PUBLIC_URL}/assets/images/logo/white.png`} alt="logo"/>
                            <p className='w-full md:w-2/3 lg:w-1/2 text-base md:text-xl lg:text-2xl font-thin text-white text-center drop-shadow-lg'>{data.subHeading}</p>
                            <img onClick={() => goToSlide(1)} className="animate-pulse h-10 duration-300 hover:scale-125 cursor-pointer drop-shadow-lg" src={`${process.env.PUBLIC_URL}/assets/images/utility/icons/white/scroll_down.png`} alt="scroll_down" />
                        </div>
                    </div>
            </div>
        )
                        ...
                        <div className='h-full w-full flex flex-col justify-end pb-10 md:pb-0'>
                            <div className={`text-white ${data.theme==='light' ? 'text-white' : 'text-black'}`}>
                                <div className='group relative pb-2 max-w-max'>
                                    <h1 className={`title uppercase font-bold text-4xl md:text-5xl lg:text-8xl max-w-min drop-shadow-lg dalay-500 ${current.current === index  ? 'animate-fadeInText opacity-100' : 'opacity-0'}`}>{data.name}</h1>
                                ...
                                <p className={`text-lg md:text-2xl font-thin mt-2 drop-shadow-lg ${current.current === index  ? 'animate-fadeInLongText opacity-100' : 'opacity-0'}`}>{data.subHeading}</p>
                                <Link tabIndex={-1} to={data.navTo} className={`flex gap-4 items-center hover:gap-12 duration-500 mt-4 ${current.current === index  ? 'animate-fadeInLong opacity-100' : 'opacity-0'} ${data.theme === 'light' ? 'text-white' : 'text-black'}`}>
